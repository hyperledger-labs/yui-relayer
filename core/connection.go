...
func createConnectionStep(src, dst *ProvableChain) (*RelayMsgs, error) {
	...
	srcConn, dstConn, err := QueryConnectionPair(sh.GetQueryProofContext(src.ChainID()), sh.GetQueryProofContext(dst.ChainID()), src, dst)
	if err != nil {
		return nil, err
	}
	...
	if !(srcConn.Connection.State == conntypes.UNINITIALIZED && dstConn.Connection.State == conntypes.UNINITIALIZED) {
		...
		srcCsRes, dstCsRes, err = QueryClientStatePair(sh.GetQueryProofContext(src.ChainID()), sh.GetQueryProofContext(dst.ChainID()), src, dst)
		...
		srcCons, dstCons, err = QueryClientConsensusStatePair(
			sh.GetQueryProofContext(src.ChainID()), sh.GetQueryProofContext(dst.ChainID()),
