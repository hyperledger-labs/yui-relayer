...
type IBCProvableQuerierI interface {
	...
	QueryClientConsensusStateWithProof(ctx QueryProofContext, dstClientConsHeight ibcexported.Height) (*clienttypes.QueryConsensusStateResponse, error)

	// QueryClientStateWithProof returns the ClientState and its proof
	QueryClientStateWithProof(ctx QueryProofContext) (*clienttypes.QueryClientStateResponse, error)

	// QueryConnectionWithProof returns the Connection and its proof
	QueryConnectionWithProof(ctx QueryProofContext) (*conntypes.QueryConnectionResponse, error)

	// QueryChannelWithProof returns the Channel and its proof
	QueryChannelWithProof(ctx QueryProofContext) (chanRes *chantypes.QueryChannelResponse, err error)

	// QueryPacketCommitmentWithProof returns the packet commitment and its proof
	QueryPacketCommitmentWithProof(ctx QueryProofContext, seq uint64) (comRes *chantypes.QueryPacketCommitmentResponse, err error)

	// QueryPacketAcknowledgementCommitmentWithProof returns the packet acknowledgement commitment and its proof
	QueryPacketAcknowledgementCommitmentWithProof(ctx QueryProofContext, seq uint64) (ackRes *chantypes.QueryPacketAcknowledgementResponse, err error)
}
...
type QueryProofContext interface {
	QueryContext
	Header() HeaderI
}

type queryProofContext struct {
	ctx    context.Context
	header HeaderI
}

var _ QueryProofContext = (*queryProofContext)(nil)

func NewQueryProofContext(ctx context.Context, header HeaderI) QueryProofContext {
	return queryProofContext{ctx: ctx, header: header}
}

func (qc queryProofContext) Context() context.Context {
	return qc.ctx
}

func (qc queryProofContext) Height() ibcexported.Height {
	return qc.header.GetHeight()
}

func (qc queryProofContext) Header() HeaderI {
	return qc.header
}
